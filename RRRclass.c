...
void train(int itermax,double *aveiter,double tol,double *errstart,double *errstop,double *trainerr)
{
int batchcount,iter,k;
double err;
...
{
char *trainfile,*testfile,*netfile,*id,logfile[50],paramfile[50];
...
if(argc==13)
	...
	{
	fprintf(stderr,"expected 12 arguments: trainfile, testfile, netfile, batchsize, epochs, itermax, tol, beta, wnorm, ynorm, margin, id\n");
	return 1;
	}
...
for(e=1;e<=epochs;++e)
	...
	train(itermax,&aveiter,tol,&errstart,&errstop,&trainerr);
